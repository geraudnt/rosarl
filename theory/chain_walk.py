...
rmin, rmax = -1, 0
p = 0.25
D, C = 2, 1-p # max([(1-p)-p,p-(1-p)]) # 0.72
...
ax.plot(p, delta_p_s0,  label=r'$\Delta P_{s_0}(\pi_1, \pi_2)$', lw = lw)
ax.plot(p, delta_p_s0_,  label=r'$\Delta P_{s_0}(\pi_2, \pi_1)$', lw = lw)
ax.plot(p, delta_p_s2,  label=r'$\Delta P_{s_2}(\pi_1, \pi_2)$', lw = lw)
ax.plot(p, C,  label=r'$C$', color="b", linestyle="dashed", lw = lw)
...
lw = 5.0
fig, ax = plt.subplots()
ax.plot(p, penalty1, label=r'$\min \{\bar R_{MIN}, (\bar R_{MIN} - \bar R_{MAX})D\}$', lw = lw)
ax.plot(p, penalty2, label=r'$\min \{\bar R_{MIN}, (\bar R_{MIN} - \bar R_{MAX})\frac{1}{C}\}$', lw = lw)
ax.plot(p, penalty, label=r'$\min \{\bar R_{MIN}, (\bar R_{MIN} - \bar R_{MAX})\frac{D}{C}\}$', lw = lw)
...
ax[0].plot(p, success, label=r'Success', lw = lw)
ax[0].plot(p, failure, label=r'Failure', linestyle="--", lw = lw)
ax[1].plot(p, success2, label=r'Success', lw = lw)
ax[1].plot(p, failure2, label=r'Failure', linestyle="--", lw = lw)
ax[2].plot(p, success1, label=r'Success', lw = lw)
ax[2].plot(p, failure1, label=r'Failure', linestyle="--", lw = lw)
# for a in ax:
#     a.label_outer()
#     a.legend()
ax[0].label_outer()
