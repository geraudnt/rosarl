...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    def build_observation_space(self):
        ...
        if self.unsafe_terminate==1:
            obs_space_dict['unsafe'] = gym.spaces.Box(-np.inf, np.inf, (1,), dtype=np.float32)
    ...
    def obs(self):
        ...
        if self.unsafe_terminate==1:
            obs['unsafe'] = np.array([self.cum_costs > 0])
        if self.observe_goal_dist:
    ...
    def step(self, action):
        ...
        # MOD: Unsafe state
        if self.unsafe_terminate==0:
            info['unsafe'] = info['cost'] > 0
        elif self.unsafe_terminate==1:
            info['unsafe'] = 0
            self.cum_costs += info['cost']
            if info['goal_met']:
                info['unsafe'] = self.cum_costs > 0
                self.done = True
                reward = reward if not info['unsafe'] else -info['unsafe']
            elif self.done:
                self.done = False
        elif self.unsafe_terminate==2:
            info['unsafe'] = info['cost'] > 0
            self.done = self.done or info['unsafe']
            reward = reward if not info['unsafe'] else -info['unsafe']
